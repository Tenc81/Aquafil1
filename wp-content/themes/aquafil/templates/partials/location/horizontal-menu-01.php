...
if(!empty($filters)) {
	...
	foreach($filters as $filter) {
		echo '
								<li class="nav__item"><a '.(is_tax("nazione-sedi") && get_queried_object_id() == $filter->term_id ? 'href="javascript:void(0);" class="active"' : 'href="'.get_term_link($filter).'"').'><span class="name">'.$filter->name.'</span> <span class="count">('.$filter->count.')</span></a></li>';
	}
	echo '
							</ul>
						</div>
